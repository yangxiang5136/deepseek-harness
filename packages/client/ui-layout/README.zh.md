...
frame 还承接一个避让接缝：`shell.overlay` 的底部住户（当前为 TaskFlow 条）把实时高度以 `--dsh-shell-bottom-clearance` 发布到 frame 元素上，frame 按该变量为列内容加底部 padding——列内容止于住户上沿，overlay 层本身仍覆盖 padding 区。变量默认 `0px`，住户销毁时移除。
